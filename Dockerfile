...
WORKDIR /src
COPY go.mod go.sum ./
# colmena is now a public, published module (github.com/mentasystems/colmena), so
# deps come straight from the module proxy — no vendoring needed. The sqlite
# driver colmena uses is pure-Go (modernc), so CGO stays disabled.
RUN go mod download
COPY *.go ./
